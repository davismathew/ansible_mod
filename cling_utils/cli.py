...
class Cling(object):
    ...
    def _dotelnetlogin(self):
        '''Spawns telnet, logins to the host
        and runs the intialisation commands'''
        # build the telnet command to spawn
        telnet_command = [self.telnet]
        telnet_command.append('%s' % (self.hostname))
        
        telnet_command = ' '.join(telnet_command)

        # spawn the process
        self.child = self._spawn(telnet_command)
        ...
        if not self.pub_key_auth:
            ...
            try:
                ...
                self.send_line(user, hide_text=True)
        ...
        if not self.pub_key_auth:
            ...
            try:
                ...
                self.send_line(self.password+"\r", hide_text=True)
                self._expect(self.prompt)
            # raise login failed if no cli prompt could be seen
            except Error as e:
                raise Error('%s: login failed (%s)' % (self.hostname, e))
    ...
    def _spawn(self, command):
        ...
        try:
            ...
            raise Error('%s: child terminated "%s %s"' % (
                self.hostname, self.child.before,command))
    ...
    def _expect(self, pattern):
        ...
        print "expecting patern ::",pattern.pattern
